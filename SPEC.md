...
A homopolymer occurs if $S_n = S_{n-1}$.
From the definition:
$$S_{n} = (S_{n-1} + I_n + 1) \pmod 4$$

If $S_n = S_{n-1}$, then $I_n + 1 \equiv 0 \pmod 4$.
But $I_n \in \{0,1,2\}$, so $I_n + 1 \in \{1,2,3\}$, which is never $0 \pmod 4$.
Therefore $S_n \neq S_{n-1}$ and homopolymers are impossible.
**Q.E.D.**
...
Expected GC Content: $P(G) + P(C) = 0.25 + 0.25 = 0.50$.
